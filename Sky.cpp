...
Sky::Sky(const wchar_t* right, const wchar_t* left, const wchar_t* up, const wchar_t* down, const wchar_t* front, 
	const wchar_t* back, Microsoft::WRL::ComPtr<ID3D11SamplerState> sampler, 
	Microsoft::WRL::ComPtr<ID3D11VertexShader> skyVS, 
	Microsoft::WRL::ComPtr<ID3D11PixelShader> skyPS, std::shared_ptr<Mesh> mesh)
{
	...
	Graphics::Device->CreateRasterizerState(&rasterDesc, this->skyRasterizer.GetAddressOf() );

	D3D11_DEPTH_STENCIL_DESC depthDesc = {};
	depthDesc.DepthEnable = true;
	depthDesc.DepthFunc = D3D11_COMPARISON_LESS_EQUAL;
	Graphics::Device->CreateDepthStencilState(&depthDesc, this->skyDepthBuffer.GetAddressOf());
...
{
	Graphics::Context->RSSetState(this->skyRasterizer.Get());
	Graphics::Context->OMSetDepthStencilState(this->skyDepthBuffer.Get(),0);

	Graphics::Context->VSSetShader(this->skyVS.Get(),0,0);
	Graphics::Context->PSSetShader(this->skyPS.Get(), 0, 0);


	SkyVSData vsData{};
	vsData.view = cam->GetViewMatrix();
	vsData.proj = cam->GetProjectionMatrix();
	Graphics::FillAndBindNextConstantBuffer(&vsData, sizeof(SkyVSData), D3D11_VERTEX_SHADER, 0);


	Graphics::Context->PSSetShaderResources(0, 1, this->skySRV.GetAddressOf()); // Bind srv to texture slot 0
	Graphics::Context->PSSetSamplers(0, 1, this->samplerOptions.GetAddressOf()); // Bind sampler
